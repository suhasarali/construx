                ...
                <Card className="col-span-1 lg:col-span-3 p-6">
                    ...
                    <div className="space-y-4">
                        {recentMaterials.length === 0 ? (
                            ...
                            <div key={m._id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg border border-border">
                                <div className="flex items-center gap-3">
                                    <div className="bg-background p-2 rounded-full shadow-sm">
                                        <Pickaxe size={16} className="text-muted-foreground" />
                                    </div>
                                    <div>
                                        <p className="font-medium text-sm text-foreground">{m.items[0]?.name}</p>
                                        <p className="text-xs text-muted-foreground">{m.items[0]?.quantity} {m.items[0]?.unit}</p>
                                    </div>
                                </div>
                                <span className={`text-xs px-2 py-1 rounded-full font-medium ${m.status === 'Approved' ? 'bg-green-500/20 text-green-500' :
                                    m.status === 'Rejected' ? 'bg-red-500/20 text-red-500' :
